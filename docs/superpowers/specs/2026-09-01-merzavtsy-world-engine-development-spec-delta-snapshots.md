...
- [ ] создать Genesis base snapshot manifest + content-addressed chunks при `WorldTime = 0`;
...
- [ ] `snapshot_pins`;
- [ ] `snapshot_heads`;
- [ ] snapshot GC/compaction journal tables;
- [ ] derived events;
- [ ] projections;
- [ ] operations journal;
...
## Этап 27. Delta snapshot/replay repository

- [ ] versioned canonical section serializer;
- [ ] deterministic stable shard/chunk boundaries;
- [ ] chunk header/file format;
- [ ] BLAKE3 content hash по uncompressed canonical bytes;
- [ ] Zstd compression;
- [ ] compressed payload checksum;
- [ ] content-addressed chunk path;
- [ ] chunk staging + fsync + atomic rename;
- [ ] Genesis base manifest;
- [ ] immutable delta manifest;
- [ ] dirty-shard tracking;
- [ ] tombstone/removal representation;
- [ ] snapshot HEAD pointer;
- [ ] atomic manifest commit;
- [ ] orphan chunk crash test;
- [ ] base + delta restore;
- [ ] canonical state hash verify;
- [ ] fallback from corrupt latest snapshot;
- [ ] max delta chain depth;
- [ ] rebase/base-manifest compaction;
- [ ] verify rebase reuses existing chunks;
- [ ] snapshot pins;
- [ ] 6h/daily/weekly retention pins;
- [ ] mark-and-sweep GC;
- [ ] GC grace period;
- [ ] GC dry-run;
- [ ] reachable-chunk safety test;
- [ ] snapshot migration provenance;
- [ ] backup/restore reachable chunks;
- [ ] delta size/dedupe metrics;
- [ ] deterministic integration test;
- [ ] module-extension delta snapshot test.
...
- [ ] snapshot GC benchmark;
- [ ] 180-world-day snapshot storage growth report;
- [ ] catch-up benchmark;
- [ ] heap profile;
